...
static BOOL SerialSend(UINT8 *pData, UINT16 length)
{
    ...
    KITL_RETAILMSG(ZONE_SEND, (
        "+SerialSend(0x%x, %d) best %d\r\n", pData, length, bestSize
    ));

// block until send is complete; no timeout
    while (total > 0) {
        toSend = (total > bestSize) ? bestSize : total;
		KITL_RETAILMSG(ZONE_SEND, ("Calling pfnSend %x %d\r\n", pData, toSend));
...
static BOOL SerialRecv(UINT8 *pData, UINT16 *pLength)
{
    ...
    while (recvCount < (sizeof(OAL_KITL_SERIAL_HEADER) + recvPayload)) {
		...
		//if (toRead == 1)
		//	KITLOutputDebugString ("toRead %d bestSize %d recvPay %d  recvCnt %d thiscnt:%d\r\n", toRead, bestSize, recvPayload, recvCount, count);
...
//(db) it appears that the sliding that is going on below results in calls to single reads to pfnRecv.
